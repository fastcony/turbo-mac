...
var _httpMap = {
  ...
  'proxy': {
    ...
    'mpp.liveapi.mgtv.com': [
      /^\/v1\/epg\/turnplay\/getLivePlayUrlMPP/i
    ],
    '182.254.116.117': [
      /^\//i
    ],
...
var _httpsMap = {
  ...
  'proxy': {
    ...
    'dmd-fifajs-h5-ikuweb-hb.youku.com': [
      /^\//i
    ],
    'acs.youku.com': [
      /^\//i
...
if (patterns.hasOwnProperty(hostname) &&
  _checkRegexList(patterns[hostname], fullUrl.slice(protoLen + hostname.length)))
    return true;
if (_checkRegexList(patterns.any, fullUrl.slice(protoLen)))
  return true;
return false;
}
...
if (_checkPatterns(urlMap.white, host, url, protoLen))
  return _direct;
if (_checkPatterns(urlMap.proxy, host, url, protoLen))
  return _proxyStr;
return false;
}
